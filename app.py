...
def mask_dataframe(df, role):
    ...
    # 관리자는 마스킹 없음 (단, Alias 컬럼은 추가해줌)
    if role == 'admin':
        if 'junkyard' in df_safe.columns:
            df_safe['real_junkyard'] = df_safe['junkyard'] # 원본 보존
            # 관리자도 화면엔 Alias로 보고싶을 수 있으니 옵션이지만, 일단 원본 유지
        return df_safe

    # 바이어/게스트 공통 마스킹
    if 'junkyard' in df_safe.columns:
        df_safe['real_junkyard'] = df_safe['junkyard'] # 내부 로직용 (화면엔 안보임)
        if role == 'buyer':
...
def log_search(keywords, s_type):
    ...
    try:
        conn = init_db()
        c = conn.cursor()
        # 간단한 위치 정보 (실제론 IP 기반 GeoIP 사용 권장)
...
df_raw = load_all_data() # 캐시된 전체 데이터
...
with st.sidebar:
    ...
    if st.session_state.user_role == 'admin':
        with st.expander("📂 Admin Tools"):
            up_files = st.file_uploader("Upload Data", type=['xlsx', 'xls', 'csv'], accept_multiple_files=True)
            if up_files and st.button("Save to DB"):
                tot = 0
                bar = st.progress(0)
    ...
    with search_tabs[2]: # 폐차장 검색
        if list_yards:
            sel_yards = st.multiselect("Junkyard Name", list_yards)
            if st.button("🔍 Search Yard", type="primary"):
                res = load_all_data()
                if sel_yards: res = res[res['junkyard'].isin(sel_yards)]
                st.session_state['view_data'] = res.reset_index(drop=True)
                st.session_state['is_filtered'] = True
                st.session_state['mode_demand'] = False
                safe_rerun()

    with search_tabs[3]: # 수요 예측
...
# [B] 재고 조회 모드 (기본)
    ...
    with main_tabs[0]:
        if df_display.empty:
            st.info("Please select filters from the sidebar.")
        else:
            # KPI
                ...
                if st.session_state.user_role == 'guest':
                    ...
                    with st.form("order_form"):
                        ...
                        if st.form_submit_button("🚀 Send Inquiry"):
                            ...
                            # 실제 이름 추적 (관리자는 원본, 바이어는 추적 시도)
                            real_name = target_partner
                            if st.session_state.user_role == 'buyer':
                                # 화면 데이터에서 원본 컬럼 찾기 (mask_dataframe에서 백업해둠)
                                try:
                                    # 해당 Alias를 가진 첫 행의 real_junkyard 값
                                    matched_row = df_display[df_display['junkyard'] == target_partner].iloc[0]
                                    if 'real_junkyard' in matched_row:
                                        real_name = matched_row['real_junkyard']
