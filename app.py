...
try:
    ...
    with st.sidebar:
        ...
        with st.expander("📂 데이터 업로드"):
            up_files = st.file_uploader("파일 (다중 가능)", type=['xlsx', 'xls', 'csv'], accept_multiple_files=True)
        ...
        with search_tabs[0]:
            if not df_all_source.empty:
                ...
                # 🟢 3. 동적 차종 목록 생성 (선택된 브랜드/연식에 맞는 차만 보임)
